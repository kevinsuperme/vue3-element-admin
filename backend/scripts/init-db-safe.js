...
async function main() {
  ...
  let connectionUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/vue3-admin';

  try {
    console.log('🔗 连接数据库...');

    // 检查是否需要使用内存数据库
    const useMemoryDB = process.env.USE_MEMORY_DB === 'true' || process.argv.includes('--memory');

    if (useMemoryDB) {
      console.log('🧠 使用内存数据库...');
      mongoServer = await MongoMemoryServer.create();
      connectionUri = mongoServer.getUri();
      console.log(`✅ 内存数据库启动成功: ${connectionUri}`);
    }
    ...
    console.log('2. 或者使用内存数据库: npm run init:db -- --memory');
    console.log('3. 检查环境变量MONGODB_URI是否正确');
    console.log('4. 安装MongoDB: https://www.mongodb.com/try/download/community');
    process.exit(1);
  } finally {
